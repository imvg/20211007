...
class spider:
    def __init__(self, ):
        try:
            ...
            self.client = CosS3Client(CosConfig(Region=region, SecretId=secret_id, SecretKey=secret_key, Timeout=60, Domain=domain), retry=3)
            self.cos_bucket = 'sourcefile-1304080031'
            self.dir = "m3u8/video/oumei/blacked"
    ...
    def controler(self):
        try:
            logging.info("登陆Blacked, 请手动输入验证码")
            self.loginBlacked()
            ...
            for k, v in zip(movList.keys(), movList.values()):
                ...
                if not ishave:
                    logging.info(f"缺少 {k} 开始抓取")
                    # res = self.checkDisk(k)
                    res = False
                    if res and "vodpath" in res.keys():
    ...
    def loginBlacked(self, url='https://login.vixen.com/i/blacked/login?'):
        try:
            ...
            while True:
                ...
                if self.browser.current_url != nowLink:
                    break
            self.browser.find_element_by_xpath('//*[@id="root"]/main/div[1]/div/a[1]').click()
            return True
        except Exception as e:
            logging.error(e)
            return False
        ...
        try:
            ...
            while True:
                movid = self.browser.find_elements_by_xpath(
                    '//*[@id="root"]/main/div[1]/div[2]/div[2]/div/div/div/div[1]/video')
                movlink = self.browser.find_elements_by_xpath(
                    '//*[@id="root"]/main/div[1]/div[2]/div[2]/div/div/div/div[1]/a')
                time.sleep(0.3)
                for id, link in zip(movid, movlink):
                    movList[(re.split('_', re.split('/', id.get_attribute('src'))[-1])[0])] = link.get_attribute('href')
                lastLink = \
                re.split('\.com', self.browser.find_elements_by_xpath('//*[@id="root"]/main/div[1]/div[3]/a')[
                    -1].get_attribute('href'))[-1]
                nowLink = re.split('\.com', self.browser.current_url)[-1]
                if lastLink != nowLink:
                    self.browser.find_elements_by_xpath('//*[@id="root"]/main/div[1]/div[3]/a')[-2].click()
                    continue
                else:
                    break
        except Exception as e:
    ...
    def checkExisted(self, vid):
        try:
            ...
            try:
                video_sql = f"select * from t_video where vnum like '%{vid}%';"
                stock_sql = f"select * from t_video_stock where vnum like '%{vid}%';"
    ...
    def downloadVod(self, url, fanhao):
        if "https://members.blacked.com/" in url:
            try:
                self.browser.get(url)
                while True:
                    try:
                ...
                while True:
                    try:
                        if 'http' not in str(vodurl):
                            vodurl = 'http://'+vodurl
                        vodres = requests.get(vodurl, headers=header, timeout=10, stream=True)
                    except Exception as e:
                        logging.warning(f"视频请求失败, 重试 {vodurl} {e}")
                        self.browser.refresh()
                        time.sleep(3)
                        continue
                    else:
                        logging.info(f"视频资源请求成功, 开始下载 {vodurl}")
                        break
                ...
                while True:
                    try:
                        ...
                        logging.warning(f"封面请求失败, 重试 {cover} {e}")
                        time.sleep(3)
                        continue
                    else:
                        logging.info(f"封面资源请求成功, 开始下载 {cover}")
                        break
    ...
    def checkDisk(self, vid):
        res = {}

        def check(vid, dir="E:\欧美\Blacked"):
    ...
    def getVodInfo(self, url):
        if "https://members.blacked.com/" in url:
            url = url.replace("https://members.blacked.com/", "https://www.blacked.com/")
            n = 0
            self.browser.get(url)
            time.sleep(3)
            while True:
    ...
    def saveBase(self, data):
        try:
            ...
            try:
                ...
                cover_uri = "/m3u8/video/oumei/blacked/" + data['fanhao'] + "/" + data['fanhao'] + "." + data[
                    'covertype']
                vod_url = "/m3u8/video/oumei/blacked/" + data['fanhao'] + "/" + data['fanhao'] + ".m3u8"
                district = "欧美"
                vnum = data['fanhao']
                status = "1"
                videotype = "1"
                ...
                with self.connection.cursor() as cursor:
                    sql = f'insert into t_video_stock(`title`,`cover_uri`,`video_uri`,`district`,`vnum`,`video_type`,`video_code`,`description`,`resolution`,`duration`,`md5`,`name`,`ch_name`,`ctime`,`status`)' \
                        f'value("{title}","{cover_uri}","{vod_url}","{district}","{vnum}","{videotype}","{videocode}","{description}","{resolution}","{duration}","","{name}","{ch_name}","{ctime}","{status}")'
                    cursor.execute(sql)
